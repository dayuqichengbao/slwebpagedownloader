...
(function () {
    ...
    // 命中小流量使用抽样样式
    var linkUrl = samNewBox ? 'https://pss.bdstatic.com/r/www/cache/static/protocol/https/soutu/css/soutu_new_sam_a6c95ec.css' : 'https://pss.bdstatic.com/r/www/cache/static/protocol/https/soutu/css/soutu_new2_baa2298.css';
    ...
    var lib = {
        ...
        base64ToFile: function (base64String) {
            try {
                ...
                var base64Data = base64String.replace(base64Regex, '');
                ...
                return new File([blob], 'filename.' + mimeType.replace('image/', ''), { type: mimeType });
            }
            catch(e) {
                return null;
            }
    ...
    var template = {
        ...
        errorHtml: function (msg, status) {
            ...
            var errorTips = hastip ? '<p class="soutu-error-tip">请使用10M以内图片，支持格式：jpg、jpeg、png、gif、bmp、webp、tif、tiff、avif、heic、heif</p>' : '';
    ...
    var soutu = {
        ...
        validate: function (file) {
            var allow = ['jpg', 'jpeg', 'png', 'bmp', 'gif', 'webp', 'tif', 'tiff', 'avif', 'heic', 'heif'];
            var arr = file && file.type && file.type.split('/') || [];
            var ext = arr.pop().toLowerCase();
            var me = this;
            me.imageType = ext;

            if (file.type === 'image/heic' || file.type === 'image/heif') {
                me.imageType = file.type.indexOf('heic') > -1 ? 'heic' : 'heif';
            }
        ...
        upload: function (file, source) {
            ...
            if (source !== 'PC_UPLOAD_SEARCH_URL') {
                ...
                if (file.type === 'image/webp') {
                    ...
                    lib.sendUbcLog('info', 'upload', {uploadType: me.uploadType, imageType: me.imageType});
                }

                // 以下格式不支持压缩，直接用源文件发起请求
                const notCompress = ['image/tif', 'image/tiff', 'image/heic', 'image/heif', 'image/avif'];

                if (notCompress.includes(file.type)) {
        ...
        uploadProcess(file, source) {
            ...
            if (file && file.type) {
                ...
                const imageType = imgType.split('/')[1];
        ...
        uploadComplete: function (res) {
            var me = this;
            if (res.status === 0) {
                lib.sendUbcLog('info', 'upload_success', {uploadType: me.uploadType, imageType: me.imageType});
                setTimeout(function () {
