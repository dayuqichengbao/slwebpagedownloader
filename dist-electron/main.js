...
import electron, { ipcMain as ipcMain$1, dialog, clipboard, shell as shell$1, app as app$1, BrowserWindow, WebContentsView } from "electron";
...
let ElectronStore$1 = class ElectronStore extends Conf {
  constructor(options) {
    let defaultCwd;
    let appVersion;
...
class ElectronStore2 {
  constructor(options) {
    __publicField(this, "store");
    this.store = new ElectronStore$1(options);
  }
  get(key) {
...
const settingsStore = new ElectronStore2({
  name: "app-config",
...
function getDownloadRootDir() {
  ...
  if (settingDir == null || settingDir.trim() == "") {
    const defaultDir = os$1.platform() === "win32" ? "C:\\Users\\Public\\Downloads" : "~/Downloads";
    return defaultDir;
  }
...
function createWindow() {
  ...
  viewA.setBounds({ x: 0, y: 0, width: 1e3, height: 800 });
  ...
  client.on("message", async (_event, method, params) => {
    ...
    try {
      ...
      win == null ? void 0 : win.webContents.send("item-add", {
        "url": url,
        "filepath": filePath
      });
    } catch {
...
ipcMain$1.handle("update-subview-url", (_, url) => {
  if (subView && url) {
...
ipcMain$1.handle("select-download-path", async () => {
  const result = await dialog.showOpenDialog({
    title: "选择下载文件夹",
    defaultPath: getDownloadRootDir(),
    properties: ["openDirectory", "createDirectory"]
  });
  settingsStore.set("downloadPath", result.canceled ? getDownloadRootDir() : result.filePaths[0]);
  return result;
});
ipcMain$1.handle("get-download-dir", () => {
  return getDownloadRootDir();
});
ipcMain$1.handle("copy-text", (_, text) => {
  clipboard.writeText(text);
});
ipcMain$1.handle("set-robots-checked", (_, value) => {
  settingsStore.set("checkRobots", value);
});
ipcMain$1.handle("get-robots-checked", () => {
...
ipcMain$1.on("shell-width-changed", (_, width) => {
  shellWidth = width + 40;
  console.log("Shell width changed:", shellWidth);
  applyLayout();
});
ipcMain$1.handle("resize-window", (_, width) => {
  const h = mainWin.getBounds().height;
  mainWin.setSize(Math.ceil(width), h);
});
ipcMain$1.handle("open-existing-folder", async (_, filepath) => {
  try {
    const result = await shell$1.openPath(filepath);
