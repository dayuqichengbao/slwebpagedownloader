    ...
    */
    function showSurvey(callback){
        ...
        var surveyhtml = surveycss + '<div id="surveywrapper"><h3>用户调查</h3><i class="c-icon c-icon-close"></i>'+
            ...
            '<input type="hidden" name="url" value="'+escapeHTML(location.href)+'"/>'+
    ...
    */
    var loadInstant = function (options) {
        ...
        $.ajax({
            "dataType": "script",
            "cache": true,
            "url":"https://pss.bdstatic.com/r/www/cache/static/protocol/https/home/js/instant_185e2be.js",
    ...
    **/
    ...
    function addPfDom() {
        ...
        $linkpref.on("mousedown",function(){
            // if (!isHitSampling('PC_AI_SEARCH_BOX_REVERSE_EXP')) {
            //     // 搜索框智能输入框设置曝光
            //     require(['modules/page-log/ubc-log'], function (Ubc) {
            //         Ubc.sendUbcLog(18463, 'show', 'search_kuang_set', {
            //             status: bds.comm.smartSearchBox ? 'aisearch' : 'default',
            //             tab: '搜索设置'
            //         });
            //     });
            // }
        ...
        if(bds && bds.comm && bds.comm.ishome && bds.comm.skin){
         ...
         if(bds.comm.ishome && window.s_domain && window.s_domain.base && window.s_domain.base == 'home'){
            ...
            $linkHistory.on("click",function(){
                ...
                setUbcLog('set', 'tj_history');
                if(bds.comm.username){
                    location.href="https://www.baidu.com/duty/privacysettings.html";
                    return false;
                }else{
        ...
        if(bds && bds.comm && bds.comm.ishome){
            var username = bds.comm.username;
            var placeholderSmaple = bds.comm.nodeSample && bds.comm.nodeSample.indexOf('AI_INPUT_PLACEHOLDER') > -1;
            if (bds.comm.skin) {
            ...
            // 首页设置，增加首页设置菜单
            if (!placeholderSmaple || (placeholderSmaple && username)) {
...
$(function(){
    ...
    function nsc(name){
        var reg = new RegExp('^\\s+|\\s+\x24'),
            key = $("#kw").get(0).value.replace(reg, '');
